...
for(year in 2017:2017){
  ...
  ndc_master_file <- paste0('C:/Users/khuon/Dropbox/Research/Summer 2021/Altruism/data/PartD/PDE/ndc_from_pde',year,'.csv')
  ...
  out_base_dir <- ensureDir(paste0('Output', year, '/'))
  ...
  wrapRDS(ndc_master, {
    ...
    if(grepl('NDCPACKAGECODE', ndc_master_line_1, fixed = T)) {
      ...
      master_source <- read_delim(ndc_master_file, delim = ndc_master_file_separator,
                                  col_types = cols_only(ndc = "c"),col_names = TRUE, n_max = ifelse(debug_mode, debug_limit, Inf))
    } else
  ...
  if(T) {
    map_outfile <- paste0(out_dir, 'ndc_atc_map', year, ' (', exec_label, ').csv')
