...
# Load the file containing TIN and the file containing PRACI
...
df_tin <- df_tin %>% gather(key=tin_pos,value=tin,tin1,tin2) 
...
#df_tin <- read_csv(file="C:/Users/khuon/Dropbox (Harvard University)/Physician Altruism/Altruism/code/TIN_xwalk_project/tin_file_sim.csv")
#df_pracid <- read_csv(file="C:/Users/khuon/Dropbox (Harvard University)/Physician Altruism/Altruism/code/TIN_xwalk_project/pracid_file_sim.csv")
#colnames(df_tin) <- c("npi","tin","year")
#colnames(df_pracid) <- c("npi","pracid","year")
#df_pracid$o_tinmask <- sapply(as.integer(df_pracid$pracid) - 100,add_zeros,4)
...
write_xwalk <- function(yr){
  ...
  # Get the set of NPIs in the MD-PPAS data and filter the HSPD data accrodingly
  unique_NPIs_tindata <- unique(df_tin_yr$npi)
  ...
  # Merge o_tinmask with pracid_info_yr 
  ...
  for(i in 1:nrow(pracid_info_yr)){
    ...
    # To speed up code: remave all the tins that have been matched and remove them from candidate pools
    matched_tins <- rbind(matched_tins,tin_comparison$tin[M])
  ...
  pracid_tin_xwalk_yr_path <- paste0("/homes/nber/lucasdo-dua28717/cutler-DUA28717/lucasdo-dua28717/Altruism/Data/TIN-PRACID-xwalk/pracid_tin_xwalk",yr,"_commonNPIsonly_LD.dta")
